...
def plot_rsi(rsi_values):
    ...
    for i, column in enumerate(rsi_values.columns[1:], start=1):
        plt.subplot(3, 1, i)  # 3 рядки, 1 колонка, номер підграфіка
        plt.plot(rsi_values['time'], rsi_values[column], label=column, color=colors[i - 1])
        plt.title(column)
        plt.legend()
        plt.xlabel('Time')
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig("rsi_plots.png")


rsi_values = get_rsi("BTCUSDT", [14, 27, 100])
plot_rsi(rsi_values)